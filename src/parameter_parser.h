 ...
 * supported
 */
enum SciDBOperation { SCIDB_OPEN, SCIDB_CREATE, SCIDB_DELETE };
 ...
 *
 * A Resolver class that resolves the string value of a parameter description to
 *address the correct parameter in the respective kind
 * of parameter. The main reason to use this class is to interprete the
 *parameter names of the create options or the connection string
 * in order to provide enums to allow a switch-case statement to assign the
 *values of an user input (options or connection string).
 *
 */
struct Resolver {
  /** a map that maps a name (string) to a generic type T (intended to be an
   * enum) */
  map<string, T> mapping;

 public:
  /**
   * @brief is the parameter name registered for the respective kind of
   *parameters
   *
   * This function checks the existance in the class' map for the enum of a
   *given
   * parameter name.
   *
   ...
   *
   * This function finds and returns the enum (key) of a given string
   *representation.
   *
 ...
 *
 * A class to parse the user input that is coming from the invoking command. The
 *user input is either
 * stated in the create/opening options or it is stated in the connection
 *string. In both cases this
 * class reads the key-value pairs and assigns it to the internal *Parameters
 *representation (e.g. scidb4gdal::CreationParameters,
...
class ParameterParser {
 public:
  ...
  /**
   * @brief checks whether the opening options or the connection string is
   *complete
   *
   * @return bool
   */
 ...
 *
 * This function will be called in from the constructor in order to parse and
 *assign the parameters to the
 ...
 */
   ...
   *
   * It validates the filename that the correct Protocol ("SCIDB:") was
   *specified.
   *
   * @return void
   */
  ...
  /**
   * @brief Splits the file name string into a connection string and a property
   *string, in which additional options are stated.
   *
   * @return void
   */
   ...
   *
   * If there are no create or opening options the file name needs to include
   *the connection parameters as key-value pairs. In
   * this case the connection string needs to contain information about the
   *login credentials as well as the access point of
   ...
   *
   * When using the opening operation it is sometimes necessary to state the
   *temporal index or the date/time for a certain
   * time slice. This temporal information can be written as syntactical sugar
   *within square brackets. This function will read
  ...
  *
  * If there are no connection parameter explicitly stated, then this function
  *will try to read the connection
  * parameter from the global environment parameter.
  *
  * @param con The connection parameters, where the information shall be stored
  *in (in/out)
  * @return void
  */
 ...
 private:
  ...
  /**
   * the connection string that is retrieved by calling
   * scidb4gdal::ParameterParser::splitPropertyString
   */
