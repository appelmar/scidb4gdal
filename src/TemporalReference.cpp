...
namespace scidb4geo
{
    ...
    void TPoint::init()
    {
        _year = _month = _week = _doy = _dow = _dom = _hour = _minute = _second =
        _fraction = -1;
    ...
    TPoint::TPoint(string str)
    {
        ...
        _pt = ptime(d, hours((_hour >= 0) ? _hour : 0) +
    ...
    TInterval::TInterval(string str)
    {
        ...
        while (pos < len) {
            if (str[pos] == 'T') {
                ...
                first_time_index = numbers.size(); //  elements of numvers and symbols
                //  with index >= first_time_index
                //  correspond to time
                ++pos;
            }
                ...
                switch (symbols[i]) {
                    ...
                    case 'M':
                        if (i >= first_time_index &&
                            first_time_index >= 0) // time -> M means minutes
    ...
    string TInterval::toStringISO()
    {
        ...
        if (_td.hours() != 0 || _td.minutes() != 0 ||
            _td.seconds() != 0) { // TODO: Add fractional seconds
            s << "T";
            if (_td.hours() != 0)
                s << _td.hours() << "H";
            if (_td.minutes() != 0)
    ...
    TInterval operator+(const TInterval& l, const TInterval& r)
    {
        ...
        res._resolution =
        (l._resolution < r._resolution) ? l._resolution : r._resolution;
        return res;
    }
    ...
    TInterval TReference::getCellsize() { return *_dt; }

    TPoint TReference::getStart() { return *_t0; }

    TPoint TReference::datetimeAtIndex(int index) { return *_t0 + index * *_dt; }
    ...
    int TReference::indexAtDatetime(TPoint& t)
    {
        ...
        if (_dt->_td.total_seconds() == 0)
            ...
            tt = dif._td.total_seconds() /
                 _dt->_td.total_seconds(); // TODO: Fractional seconds?
