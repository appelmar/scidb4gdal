...
*stored in SciDB.
*
* A structure for storing general metadata of an array in SciDB. The most basic
*representation is considered
* to hold at least information about the array name, its attributes and its
*dimensions.
*
* Dimensions:
* The dimensions describe the conditions unber which the information was stored.
*For example dimensions be
* temporal, spatial (e.g. image coordinates) or they describe other attributal
*information (e.g. gender, a category, a sensor...)
*
* Attributes:
...
*the source
* file or from the SciDB database. This reference is restricted to a two
*dimensional projection
* with the dimension x and y. X can be thought of the west-east direction and Y
*is the north-south
* direction.
*/
...
/**
  ...
  *SciDB
  *
  * This structure holds information about the temporal reference of the array
  *in SciDB. It inherits
  * from the class scidb4geo::TReference, which is also used in the SciDB plugin
  *for transforming between dates
  * or date time statements into the discrete dimension value used in the SciDB
  *array.
  *
  * Similar to scidb4gdal::SciDBSpatialReference this struct simply extends the
...
struct SciDBTemporalReference : public TReference {
   ...
   *(t0 value) and the intented
   * temporal resolution by stating a temporal interval statement. The mentioned
   *strings are passed to the
   * parent class scidb4geo::TReference.
   *
   * @param t0text temporal datum of the data set as a ISO 8601 conform date or
   *date-time string, e.g. "2004-06-14" (date),"2004-06-14T23:34:30" (date-time)
   *or "2004W28" (weeks)
   ...
   *not set. To do this after
   * creating the object this function will achieve this by taking valid ISO
   *8601 strings.
   *
   * @param t0 temporal datum of the data set as a ISO 8601 conform date or
   *date-time string, e.g. "2004-06-14" (date),"2004-06-14T23:34:30" (date-time)
   *or "2004W28" (weeks)
   * @param dt interval statement conform to ISO 8601 time periods, e.g. "P1D"
   *(one day interval) or "P14D" (14 days)
   *
   * @return void
   */
   ...
   *temporal datum with the stated temporal point.
   *
   * @return void
   */
   ...
   *the temporal interval with the steated temporal interval.
   *
   * @return void
   */
   ...
   *temporal datum of the SciDB array.
   *
 ...
 *array.
 *
 * This structure represents a SciDB array with a temporal reference. By
 *inheriting from SciDBArray and
 * SciDBTemporalReference the structure allows to identify the temporal
 *dimension in GDAL, as well as holding
 * information about the temporal reference used in SciDB and it allows also the
 *translation between
...
struct SciDBTemporalArray : public virtual SciDBArray,
   ...
   *assigned to be the temporal dimension. If the
   ...
   *temporal dimension.
   *
   * Returns the index in SciDBArray's dimension list that represents the
   *temporal dimension.
   *
 ...
 protected:
   ...
   *temporal dimension at initialization. It
   * will construct a new "temporal" dimension and stores it in the dimension
   *list and it saves the position index of this dimension
   * in the list.
   *
   * @return void
   */
   ...
   *dimensions, by looping through that list
   * and searching for the dimension that is using the prior specified name of
   *the dimension or the default temporal
   * dimension name.
   *
   * @return void
   */
 ...
 *array.
 *
 * The SciDBSpatial array extends the SciDBArray with a SciDBSpatialReference,
 *meaning that some dimensions
 * will be assigned to refer to a West-East dimension and a North-South
 *dimension.
 *
...
struct SciDBSpatialArray : public virtual SciDBArray,
  ...
  */
   ...
   *order to create a SciDBSpatialArray.
   *
   ...
   *dimensions of the array (scidb4gdal::SciDBArray::dims).
   *
   ...
   *dimension list
   *
   * This function returns the position of the y dimension in the list of
   *dimensions of the array (scidb4gdal::SciDBArray::dims).
   *
 ...
 protected:
   ...
   *two representation of spatial dimensions (x and y). The function
   * also adds those SciDBDimensions to the dimension list and stores their
   *positions in that list to access them later.
   *
   * @return void
   */
   ...
   *assigned names of the
   * x and y dimension. If there is no differing dimension name statement, then
   *the default names
 ...
 *temporal array
 *
 * This structure simply combines the SciDBSpatialArray and the
 *SciDBTemporalArray to hold information on
...
struct SciDBSpatioTemporalArray : public SciDBSpatialArray,
   ...
   *system and a specified temporal reference. The temporal reference
   * is established by stating the temporal datum and the temporal resolution as
   *ISO 8601 strings.
   *
   * @param t0text temporal datum of the data set as a ISO 8601 conform date or
   *date-time string, e.g. "2004-06-14" (date),"2004-06-14T23:34:30" (date-time)
